...
class HomeRateCustomerModal extends StatefulWidget {
  final OrderDetailData order;

  const HomeRateCustomerModal({Key? key, required this.order}) : super(key: key);

  @override
  State<HomeRateCustomerModal> createState() => _HomeRateCustomerModalState();
}

class _HomeRateCustomerModalState extends State<HomeRateCustomerModal> {
  double _rate = 0;
  String _note = '';
          ...
          Consumer(
            builder: (context, ref, child) => CustomButton(
              ...
              onPressed: () => ref.read(userReviewProvider.notifier).addReview(
                    ...
                    success: () {
                      ref.read(homeProvider.notifier).updateState(ref.watch(orderProvider).totalActiveOrder);
