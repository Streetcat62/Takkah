...
class FilteredShopsInPickupNotifier
    extends StateNotifier<FilteredShopsInPickupState> {
  final ShopsRepository _shopsRepository;
  int _filteredPage = 0;

  FilteredShopsInPickupNotifier(this._shopsRepository)
      : super(const FilteredShopsInPickupState());
    ...
    if (connected) {
      if (_filteredPage == 0) {
        ...
        final response = await _shopsRepository.getFilteredShops(
          page: ++_filteredPage,
          groupId: groupId,
          deliveryType: ShopDeliveryType.pickup,
        );
        response.when(
          success: (data) {
