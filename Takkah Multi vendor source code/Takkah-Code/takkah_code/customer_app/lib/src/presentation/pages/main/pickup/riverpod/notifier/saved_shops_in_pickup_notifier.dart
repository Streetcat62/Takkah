...
class SavedShopsInPickupNotifier
    extends StateNotifier<SavedShopsInPickupState> {
  final ShopsRepository _shopsRepository;

  SavedShopsInPickupNotifier(this._shopsRepository)
      : super(const SavedShopsInPickupState());
      ...
      response.when(
        success: (data) {
          ...
          for (final shop in (data.data ?? <ShopData>[])) {
            if (AppHelpers.checkHasPickup(shop.deliveries)) {
              shops.add(shop);
            }
          }
