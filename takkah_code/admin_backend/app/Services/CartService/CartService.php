        ...
        if ($cart) {
            try {
                $cartId = DB::transaction(function () use ($collection, $cart, $shopProduct, $user) {
                    ...
                    $userCart = UserCart::firstWhere('uuid',data_get($collection,'user_cart_uuid',null));
...
//                    'discount' => data_get($collection, 'discount', 0),
    ...
    public function cartProductDelete(int $id): array
    {
        ...
        if ($cartDetail) {
            ...
            $cart = $this->calculatePrice($cartDetail->userCart?->cart?->id);
...
//        }
    ...
    private function checkBonusShop(CartDetail $cartDetail, BonusShop $bonusShop)
    {
        $cartDetail->userCart->cartDetails()->updateOrCreate([
