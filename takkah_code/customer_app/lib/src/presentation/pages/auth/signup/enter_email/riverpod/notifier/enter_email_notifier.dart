...
class EnterEmailNotifier extends StateNotifier<EnterEmailState> {
  final AuthRepository _authRepository;
  final UserRepository _userRepository;
  String _email = '';

  EnterEmailNotifier(this._authRepository, this._userRepository)
      : super(const EnterEmailState());
  ...
  Future<void> registerWithApple(BuildContext context) async {
    ...
    try {
      ...
      response.when(
        ...
        failure: (fail, errorData) {
          state = state.copyWith(isAppleLoading: false);
          debugPrint('===> register apple fail $fail');
        },
      );
    } catch (e) {
  ...
  Future<void> sendEmailOtp({
    ...
    if (await AppConnectivity.connectivity()) {
      ...
      final response = await _authRepository.sendEmailOtp(email: _email);
      response.when(
        success: (data) {
          state = state.copyWith(isLoading: false);
          success?.call(_email);
        ...
        failure: (fail, errorData) {
          ...
          state = state.copyWith(isLoading: false);
          debugPrint('==> send email otp fail: $fail');
          error?.call();
        },
      );
    } else {
